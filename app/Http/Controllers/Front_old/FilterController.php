    ...
    public function index(Request $request, $dcat) {
        ...
        if ($request->ajax()) {
            ...
            if($cat != '' && $cat != 'all'){
                ...
                if($id){
                    $cat_id = $id->id;
                    $prods = $prods->where(function ($query) use ($cat_id){
                      $query->where('category_id', $cat_id)->orWhere('sub_category_id', $cat_id)->orWhere('sub_category_child_id', $cat_id);
                   });
                }
            }
            ...
            //$prods = $prods->select('products.*')->groupBy('id');
            $prods = $prods->select('products.*')->groupBy('id')->paginate(24);
            
           /* echo "<pre>";
            print_r($prods->toSql());
            print_r($prods->getBindings());
            die;
            */
            $i = 0;
            $searchproducts = [];
        ...
        if($categorysearch != 'all'){
                ...
                if($id){
                    ...
                    $products = $products->where('category_id', $id->id)->where('products.status',1)->orWhere('sub_category_id', $id->id)->orWhere('sub_category_child_id', $id->id);
