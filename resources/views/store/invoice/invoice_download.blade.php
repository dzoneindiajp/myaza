 ...
 <div class="wrapper"
   ...
   <div class="header"
     ...
     <div class="logo" style="  float: left;">
          <a style="text-decoration: none;;" href="{{ url('/') }}"> <img style=" max-width: 120px; height: auto;" src="{{ asset('images/logo.png') }}" style=" max-height: 115px;">
       </a> </div>
   ...
   <div class="mid-content"
     ...
     <div class="product-info">
       ...
       <table
         ...
         <tbody>
           <tr align="left">
             <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;">{{ $single->users ? $single->users->name : "" }}<br>
               Email: {{ $single->users ? $single->users->email : "" }} Phone: {{ $single->users ? $single->users->mobile : "" }}<br>
...
<table style=" background-color: #fff;  border-collapse: collapse;  box-shadow: 0 2px 4px 0 rgba(0, 0, 0, 0.08);  color: #333333;  font-size: 14px;  margin: auto;  padding: 5px;  text-decoration: none;  width: 100%; margin: 0 0 25px; ">
       ...
       @if ($lists)
           @foreach ($lists as $list)
               ...
               @endphp
               <tr>
                   <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;" style="height:60px;width:60px"><img style="width: 75px; max-width: 100%; height: auto;" src="{{ asset('file/'. $list->images)}}"> </td>
                   <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;">
                     <p style=" margin-top: 0;">{{$product->name}} </p>
                     <p style=" margin-top: 0;">Size : {{ $list->size ? $list->size->name : ""}}</p>
                     <p style=" margin-top: 0;">Color : {{ $list->color ? $list->color->name : "" }}</p>
                   </td>
                   <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;">{{ $list->qty }}</td>
                   <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;">{{$list->sale_price}}</td>
                 </tr>
           @endforeach
       @endif
       ...
       <tfoot>
           <tr>
             ...
             <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;" class="text-right"><span class="price-new"> ₹   {{$single->total_amount}}</span> </td>
           </tr>
           <tr>
     ...
     <table
       ...
       <tbody>
         <tr>
           ...
           <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;">{{$single->users ? $single->users->name : "" }}</td>
         </tr>
         {{-- <tr>
         ...
         <tr>
           ...
           <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;"> {{ $single->users && $single->users->city ? $single->users->city->name : "" }}</td>
         </tr>
         <tr>
           <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;">State</td>
           <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;"> {{ $single->users && $single->users->stat ? $single->users->stat->name : "" }}</td>
         </tr>
         <tr>
           <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;">Zip code</td>
           <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;"> {{ $single->users ? $single->users->postcode : "" }}</td>
         </tr>
         <tr>
           <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;">Phone</td>
           <td style="border: 1px solid #e0e0e0;  padding: 7px 10px;"> {{ $single->users ? $single->users->mobile : "" }}</td>
         </tr>
         {{-- <tr>
