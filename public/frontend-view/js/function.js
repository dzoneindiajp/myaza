(function ($) {
	...
	$('.slider-carousel').on('init', function() {
	    if($('.slider-carousel').find('.slick-current').find('video').length !== 0) {
			var cid = '';
			cid = $('.slider-carousel').find('.slick-current').find('video').attr('id');
			$(".slider-carousel .slick-current video")[0].play();
			document.getElementById(cid).addEventListener('ended', myHandler, false);
		}
	});
...
$('.carousel .carousel-item').each(function(){
	...
	$('.testimonial-carousel').slick({ 
		...
		$(document).on('click', '.filter-sidebar .product-widget-title', function(){
			$( this ).toggleClass('open');	
			$( this ).parents(".product-widget").toggleClass('open');
		});
		...
		if($('#cart_group').length>	0)	{
			$(document).on('click','.qty-btn',function(e){
				...
				var oldValue = parent.find('.qty').val(),
					newVal = 1;
				var totalinvent = parent.find('.qty').attr('maxlength');		
				var isChanged = 0;
				if($(this).hasClass('inc')) {
					newVal = parseInt(oldValue) + 1;
					if(parseInt(newVal) <= parseInt(totalinvent)){
						isChanged = 1;
						newVal = newVal;
					}else{
						newVal = oldValue;
				...
				if(isChanged ==1 && $('#cart_group').length>0){
					var pid = parent.find('.remove-item').attr('data-product_id');
					updateCartItemQuantity(newVal,pid,parent);
				}
		});
		}
		// Quantity Button plus minus	
		if($('#grouped-add-to-cart').length>0)	{
